...
# date: 7/24/19
# version: 1.0
# description: This program builds a multi-layer perceptron model and saves it to a .dat file
################################################################################################################################################
...
########################################################################################################################################################################
# build_mlp(X_train, y_train)
# This function builds a multi layer perceptron using x_train and y_train
# inputs: X_train- the feature vector, y_train- the label vector
# returns: clf- an mlp model
#############################################################################################################################################

def build_mlp(X_train, y_train):
	clf = MLPClassifier(solver = 'lbfgs', alpha = 1e-5, hidden_layer_sizes = (100,), random_state=1, max_iter=500)
...
# This function uses the validation feature vector (X_test) and the generated mlp model(clf) to predict the label of the validation feature vector
# inputs: row
# returns: clf predictions
#############################################################################################################################################
...
def write_to_file(y_pred, TP, TN, foldnum, acc):
	...
	file_to_open = "fold" + str(foldnum) + "_hog_results_MLP.txt"
...
def save_to_pickle(i, clf):
	filename = "MLP_hog_model_" + str(i) +".sav"
